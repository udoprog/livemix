...
struct control {
	...
	uint32_t container;
	struct spa_list link;
	struct lm_stream_control control;
	struct spa_pod *info;
		...
		uint32_t id, uint32_t flags, const struct spa_pod *param)
{
	...
	case SPA_PARAM_Buffers:
		if (impl->direction == SPA_DIRECTION_INPUT &&
		    SPA_FLAG_IS_SET(impl->flags, LM_STREAM_FLAG_MAP_BUFFERS))
...
static bool stream_set_state(struct lm_stream *stream, enum lm_stream_state state,
		int res, const char *error)
{
	enum lm_stream_state old = stream->state;
	...
	if (changed) {
		...
		pw_log_debug("%p: update state from %s -> %s (%d) %s", stream,
			     lm_stream_state_as_string(old),
			     lm_stream_state_as_string(state), res, stream->error);

		if (state == LM_STREAM_STATE_ERROR) {
			pw_log_error("%p: error (%d) %s", stream, res, error);
			errno = -res;
		}

		stream->state = state;
		lm_stream_emit_state_changed(stream, old, state, error);
	}
	return changed;
}

static struct buffer *get_buffer(struct lm_stream *stream, uint32_t id)
{
...
static inline void call_process(struct stream *impl)
{
	...
	if (impl->rt_callbacks.funcs && !impl->disconnecting)
		spa_callbacks_call_fast(&impl->rt_callbacks, struct lm_stream_events, process, 0);
}
                 ...
                 bool async, uint32_t seq, const void *data, size_t size, void *user_data)
{
	...
	struct lm_stream *stream = &impl->this;
	pw_log_trace_fp("%p: drained", stream);
	lm_stream_emit_drained(stream);
                 ...
                 bool async, uint32_t seq, const void *data, size_t size, void *user_data)
{
	...
	lm_stream_emit_trigger_done(stream);
	return 0;
}
...
static void call_trigger_done(struct stream *impl)
{
	...
	if (impl->trigger_done_rt) {
		if (impl->rt_callbacks.funcs)
			spa_callbacks_call(&impl->rt_callbacks, struct lm_stream_events, trigger_done, 2);
	} else {
		pw_loop_invoke(impl->main_loop,
		...
		uint32_t id, const struct spa_pod *param)
{
	...
	if (impl->in_emit_param_changed++ == 0)
		lm_stream_emit_param_changed(stream, id, param);
...
static int impl_set_param(void *object, uint32_t id, uint32_t flags, const struct spa_pod *param)
{
	...
	if (stream->state == LM_STREAM_STATE_ERROR)
...
static int impl_send_command(void *object, const struct spa_command *command)
{
	...
	case SPA_NODE_COMMAND_Pause:
		...
		if (stream->state == LM_STREAM_STATE_STREAMING && id != SPA_NODE_COMMAND_Flush) {
			pw_log_debug("%p: pause", stream);
			stream_set_state(stream, LM_STREAM_STATE_PAUSED, 0, NULL);
		}
		break;
	case SPA_NODE_COMMAND_Start:
		if (stream->state == LM_STREAM_STATE_PAUSED) {
	...
	lm_stream_emit_command(stream, command);
	return 0;
}
...
static void clear_buffers(struct lm_stream *stream)
{
	struct stream *impl = SPA_CONTAINER_OF(stream, struct stream, this);
	uint32_t i, j;
	...
	for (i = 0; i < impl->n_buffers; i++) {
		...
		if (SPA_FLAG_IS_SET(b->flags, BUFFER_FLAG_ADDED))
			lm_stream_emit_remove_buffer(stream, &b->this);
...
static int parse_latency(struct lm_stream *stream, const struct spa_pod *param)
{
	struct stream *impl = SPA_CONTAINER_OF(stream, struct stream, this);
		...
		struct spa_buffer **buffers, uint32_t n_buffers)
{
	...
	for (i = 0; i < n_buffers; i++) {
		...
		if (SPA_FLAG_IS_SET(impl_flags, LM_STREAM_FLAG_MAP_BUFFERS)) {
	...
	for (i = 0; i < n_buffers; i++) {
		...
		lm_stream_emit_add_buffer(stream, &b->this);
	}
	return 0;
}
...
static void proxy_error(void *_data, int seq, int res, const char *message)
{
	 ...
	 * do a lm_stream_set_error() */
	errno = -res;
	lm_stream_emit_state_changed(stream, stream->state,
			LM_STREAM_STATE_ERROR, message);
	errno = old_errno;
}
...
static struct control *find_control(struct lm_stream *stream, uint32_t id)
{
	struct control *c;
...
static int node_event_param(void *object, int seq,
		...
		struct spa_pod *param)
{
	struct lm_stream *stream = object;
	...
	case SPA_PARAM_Props:
	{
		...
		SPA_POD_OBJECT_FOREACH(obj, prop) {
			...
			lm_stream_emit_control_info(stream, prop->key, &c->control);
		}
		break;
	}
	default:
		...
		enum pw_node_state state, const char *error)
{
	...
	case PW_NODE_STATE_RUNNING:
		if (stream->state == LM_STREAM_STATE_PAUSED)
			stream_set_state(stream, LM_STREAM_STATE_STREAMING, 0, NULL);
		break;
	case PW_NODE_STATE_ERROR:
		stream_set_state(stream, LM_STREAM_STATE_ERROR, -EIO, error);
		break;
	default:
		break;
	}
...
static void on_core_error(void *data, uint32_t id, int seq, int res, const char *message)
{
	...
	if (id == PW_ID_CORE && res == -EPIPE) {
		stream_set_state(stream, LM_STREAM_STATE_UNCONNECTED, res, message);
	}
}
		...
		const char *val, size_t len)
{
	...
	struct lm_stream *this = match->stream;
...
SPA_EXPORT
struct lm_stream * lm_stream_new(struct pw_core *core, const char *name,
...
SPA_EXPORT
struct lm_stream *
lm_stream_new_simple(struct pw_loop *loop,
		     const char *name,
		     struct pw_properties *props,
		     ...
		     void *data)
{
	...
	lm_stream_add_listener(this, &impl->data.stream_listener, events, data);
...
{
	switch (state) {
	case LM_STREAM_STATE_ERROR:
		return "error";
	case LM_STREAM_STATE_UNCONNECTED:
		return "unconnected";
	case LM_STREAM_STATE_CONNECTING:
		return "connecting";
	case LM_STREAM_STATE_PAUSED:
		return "paused";
	case LM_STREAM_STATE_STREAMING:
		return "streaming";
	}
...
SPA_EXPORT int
lm_stream_connect(struct lm_stream *stream,
		  ...
		  uint32_t n_params)
{
	...
	if (stream->node != NULL || stream->state != LM_STREAM_STATE_UNCONNECTED)
		return -EBUSY;
	...
	impl->trigger_done_rt = SPA_FLAG_IS_SET(flags, LM_STREAM_FLAG_RT_TRIGGER_DONE);
	impl->early_process = SPA_FLAG_IS_SET(flags, LM_STREAM_FLAG_EARLY_PROCESS);
	...
	if (SPA_FLAG_IS_SET(flags, LM_STREAM_FLAG_ASYNC))
	...
	if (SPA_FLAG_IS_SET(flags, LM_STREAM_FLAG_ALLOC_BUFFERS))
	...
	stream_set_state(stream, LM_STREAM_STATE_CONNECTING, 0, NULL);
	...
	if (flags & LM_STREAM_FLAG_AUTOCONNECT)
	...
	if (flags & LM_STREAM_FLAG_EXCLUSIVE)
	...
	if (flags & LM_STREAM_FLAG_DONT_RECONNECT)
	...
	if (!SPA_FLAG_IS_SET(flags, LM_STREAM_FLAG_RT_PROCESS)) {
	...
	if (flags & LM_STREAM_FLAG_DRIVER)
		pw_properties_set(stream->properties, PW_KEY_NODE_DRIVER, "true");
	if (flags & LM_STREAM_FLAG_TRIGGER) {
...
SPA_EXPORT
uint32_t lm_stream_get_node_id(struct lm_stream *stream)
{
...
SPA_EXPORT
int lm_stream_set_error(struct lm_stream *stream,
			int res, const char *error, ...)
{
	struct stream *impl = SPA_CONTAINER_OF(stream, struct stream, this);
	...
	if (res < 0) {
		...
		stream_set_state(stream, LM_STREAM_STATE_ERROR, res, value);
	}
	return res;
}

SPA_EXPORT
int lm_stream_update_params(struct lm_stream *stream,
			const struct spa_pod **params,
			uint32_t n_params)
{
	struct stream *impl = SPA_CONTAINER_OF(stream, struct stream, this);
...
SPA_EXPORT
int lm_stream_set_param(struct lm_stream *stream, uint32_t id, const struct spa_pod *param)
{
	struct stream *impl = SPA_CONTAINER_OF(stream, struct stream, this);
	ensure_loop(impl->main_loop, return -EIO);
...
{
	return lm_stream_get_time_n(stream, time, sizeof(struct old_time));
}
