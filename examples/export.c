...
static int make_node(struct data *data)
{
        ...
        struct pw_properties *adapter_props;
        ...
        adapter_props = pw_properties_copy(props);

        pw_properties_setf(adapter_props, "adapt.follower.spa-node", "pointer:%p", &data->impl_node);
        pw_properties_set(adapter_props, "object.register", "false");

        data->adapter_node = pw_impl_factory_create_object(factory, NULL, PW_TYPE_INTERFACE_Node, PW_VERSION_NODE, adapter_props, 0);

        if (data->adapter_node == NULL) {
                return -errno;
        }
    
        pw_core_export(data->core, PW_TYPE_INTERFACE_Node, &props->dict, data->adapter_node, 0);
...
int main(int argc, char *argv[])
{
        ...
        data.params[3] = SPA_PARAM_INFO(SPA_PARAM_Format, SPA_PARAM_INFO_READ);
        data.params[4] = SPA_PARAM_INFO(SPA_PARAM_Buffers, 0);
        data.info.params = data.params;
        data.info.n_params = 5;
        data.format = (struct spa_audio_info_raw) {
            .format = SPA_AUDIO_FORMAT_S16,
            .rate = 44100,
            .channels = 2,
        };
        // data.format = SPA_AUDIO_INFO_RAW_INIT(SPA_AUDIO_FORMAT_S16, 0, 44100, 2, {0});
