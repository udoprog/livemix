...
#ifndef LIVEMIX_STREAM_H
#define LIVEMIX_STREAM_H
 ...
 *
 * \ref lm_stream "Streams" are used to exchange data with the
 ...
 *
 * Make a new stream with \ref lm_stream_new(). You will need to specify
 ...
 * \ref lm_stream_connect(). Pass the desired direction as an argument.
 *
 * The direction is:
 ...
 * PipeWire node, use the \ref LM_STREAM_FLAG_AUTOCONNECT and set the
 ...
 * with a call to \ref lm_stream_update_params().
 *
 * As arguments to \ref lm_stream_update_params() an array of spa_param
 ...
 * \ref LM_STREAM_STATE_PAUSED state.
 *
 * \section sec_streaming Streaming
 *
 * From the \ref LM_STREAM_STATE_PAUSED state, the stream can be set to
 * the \ref LM_STREAM_STATE_STREAMING state by the PipeWire server when
 ...
 * consumed.
 *
 * \ref lm_stream_dequeue_buffer() should be used to get the data and
 ...
 *
 * When the buffer has been processed, call \ref lm_stream_queue_buffer()
 * to let PipeWire reuse the buffer.
 *
 * Although not strictly required, it is recommended to call \ref
 * lm_stream_dequeue_buffer() and lm_stream_queue_buffer() from the
 ...
 *
 * When the LM_STREAM_FLAG_RT_PROCESS flag was given, this function will be
 ...
 *
 * \ref lm_stream_dequeue_buffer() gives an empty buffer that can be filled.
 *
 * The buffer is owned by the stream and stays alive until the
 * remove_buffer event is emitted or the stream is destroyed.
 *
 * Filled buffers should be queued with \ref lm_stream_queue_buffer().
 *
 * Although not strictly required, it is recommended to call \ref
 * lm_stream_dequeue_buffer() and lm_stream_queue_buffer() from the
 * process() callback to minimize the amount of buffering and
 ...
 *
 * With \ref lm_stream_get_time_n() and lm_stream_get_nsec() on can get accurate
 ...
 * \ref LM_STREAM_STATE_STREAMING state and \ref lm_stream_is_driving() returns
 * true. It must then use lm_stream_trigger_process() to start the graph
 * cycle.
 *
 * \ref lm_stream_trigger_process() will result in a process event, where a buffer
 ...
 * thread, fill it with data and then call \ref lm_stream_trigger_process() to
 ...
 * \ref lm_stream_trigger_process() to start the graph and will dequeue, process
 ...
 * \ref lm_stream_trigger_process(). This will result in a RequestProcess command
 * in the driver stream. If the driver supports this, it can then perform
 * \ref lm_stream_trigger_process() to start the actual graph cycle.
 *
 * \section sec_stream_disconnect Disconnect
 *
 * Use \ref lm_stream_disconnect() to disconnect a stream after use.
 *
...
/** \enum lm_stream_state The state of a stream */
enum lm_stream_state {
	LM_STREAM_STATE_ERROR = -1,		/**< the stream is in error */
	LM_STREAM_STATE_UNCONNECTED = 0,	/**< unconnected */
	LM_STREAM_STATE_CONNECTING = 1,		/**< connection is in progress */
	LM_STREAM_STATE_PAUSED = 2,		/**< paused */
	LM_STREAM_STATE_STREAMING = 3		/**< streaming */
};

/** a buffer structure obtained from lm_stream_dequeue_buffer(). The size of this
...
struct pw_buffer {
	...
	uint64_t time;			/**< For capture streams, this field contains the
					  ...
					  *  the pw_time values or lm_stream_get_nsec()
					  *  Since 1.0.5 */
};

struct lm_stream_control {
...
/** A time structure.
 *
 * Use lm_stream_get_time_n() to get an updated time snapshot of the stream.
 ...
 *
 *\code{.c}
 *    uint64_t now = lm_stream_get_nsec(stream);
...
struct pw_time {
	int64_t now;			/**< the time in nanoseconds. This is the time when this
					  ...
					  *  graph cycle. You can use lm_stream_get_nsec() to
...
struct lm_stream_events {
#define PW_VERSION_STREAM_EVENTS	2
	uint32_t version;

	void (*destroy) (void *data);
	/** when the stream state changes. Since 1.4 this also sets errno when the
	 * new state is LM_STREAM_STATE_ERROR */
	void (*state_changed) (void *data, enum lm_stream_state old,
				enum lm_stream_state state, const char *error);

	/** Notify information about a control.  */
	void (*control_info) (void *data, uint32_t id, const struct lm_stream_control *control);
...
const char * lm_stream_state_as_string(enum lm_stream_state state);

/** \enum lm_stream_flags Extra flags that can be used in \ref lm_stream_connect() */
enum lm_stream_flags {
	LM_STREAM_FLAG_NONE = 0,			/**< no flags */
	LM_STREAM_FLAG_AUTOCONNECT	= (1 << 0),	/**< try to automatically connect
							  *  this stream */
	LM_STREAM_FLAG_INACTIVE		= (1 << 1),	/**< start the stream inactive,
							  *  lm_stream_set_active() needs to be
							  *  called explicitly */
	LM_STREAM_FLAG_MAP_BUFFERS	= (1 << 2),	/**< mmap the buffers except DmaBuf that is not
							  *  explicitly marked as mappable. */
	LM_STREAM_FLAG_DRIVER		= (1 << 3),	/**< be a driver */
	LM_STREAM_FLAG_RT_PROCESS	= (1 << 4),	/**< call process from the realtime
							  *  thread. You MUST use RT safe functions
							  *  in the process callback. */
	LM_STREAM_FLAG_NO_CONVERT	= (1 << 5),	/**< don't convert format */
	LM_STREAM_FLAG_EXCLUSIVE	= (1 << 6),	/**< require exclusive access to the
							  *  device */
	LM_STREAM_FLAG_DONT_RECONNECT	= (1 << 7),	/**< don't try to reconnect this stream
							  *  when the sink/source is removed */
	LM_STREAM_FLAG_ALLOC_BUFFERS	= (1 << 8),	/**< the application will allocate buffer
							  *  memory. In the add_buffer event, the
							  *  data of the buffer should be set */
	LM_STREAM_FLAG_TRIGGER		= (1 << 9),	/**< the output stream will not be scheduled
	...
	LM_STREAM_FLAG_ASYNC		= (1 << 10),	/**< Buffers will not be dequeued/queued from
	...
	LM_STREAM_FLAG_EARLY_PROCESS	= (1 << 11),	/**< Call process as soon as there is a buffer
	...
	LM_STREAM_FLAG_RT_TRIGGER_DONE	= (1 << 12),	/**< Call trigger_done from the realtime
							  *  thread. You MUST use RT safe functions
							  *  in the trigger_done callback. Since 1.1.0 */
};

/** Create a new unconnected \ref lm_stream
 * \return a newly allocated \ref lm_stream */
struct lm_stream *
lm_stream_new(struct pw_core *core,		/**< a \ref pw_core */
	      const char *name,			/**< a stream media name */
	      struct pw_properties *props	/**< stream properties, ownership is taken */);

struct lm_stream *
lm_stream_new_simple(struct pw_loop *loop,	/**< a \ref pw_loop to use as the main loop */
		     const char *name,		/**< a stream media name */
		     struct pw_properties *props,/**< stream properties, ownership is taken */
		     const struct lm_stream_events *events,	/**< stream events */
		     void *data					/**< data passed to events */);

/** Destroy a stream */
void lm_stream_destroy(struct lm_stream *stream);

void lm_stream_add_listener(struct lm_stream *stream,
			    struct spa_hook *listener,
			    const struct lm_stream_events *events,
			    void *data);

/** Get the current stream state. Since 1.4 this also sets errno when the
 * state is LM_STREAM_STATE_ERROR */
enum lm_stream_state lm_stream_get_state(struct lm_stream *stream, const char **error);

const char *lm_stream_get_name(struct lm_stream *stream);

struct pw_core *lm_stream_get_core(struct lm_stream *stream);

const struct pw_properties *lm_stream_get_properties(struct lm_stream *stream);

int lm_stream_update_properties(struct lm_stream *stream, const struct spa_dict *dict);

/** Connect a stream for input or output on \a port_path.
 * \return 0 on success < 0 on error.
 *
 * You should connect to the process event and use lm_stream_dequeue_buffer()
 * to get the latest metadata and data. */
int
lm_stream_connect(struct lm_stream *stream,		/**< a \ref lm_stream */
		  ...
		  uint32_t target_id,			/**< should have the value PW_ID_ANY.
							  ...
							  */
		  enum lm_stream_flags flags,		/**< stream flags */
 ...
 * \return node ID. */
uint32_t
lm_stream_get_node_id(struct lm_stream *stream);

/** Disconnect \a stream  */
int lm_stream_disconnect(struct lm_stream *stream);

/** Set the stream in error state */
int lm_stream_set_error(struct lm_stream *stream,	/**< a \ref lm_stream */
...
int
lm_stream_update_params(struct lm_stream *stream,	/**< a \ref lm_stream */
			const struct spa_pod **params,	/**< an array of params. */
			uint32_t n_params		/**< number of elements in \a params */);

/**
 * Set a parameter on the stream. This is like lm_stream_set_control() but with
...
int lm_stream_set_param(struct lm_stream *stream,	/**< a \ref lm_stream */
			uint32_t id,			/**< the id of the param */
			const struct spa_pod *param	/**< the params to set */);

/** Get control values */
const struct lm_stream_control *lm_stream_get_control(struct lm_stream *stream, uint32_t id);

/** Set control values */
int lm_stream_set_control(struct lm_stream *stream, uint32_t id, uint32_t n_values, float *values, ...);

/** Query the time on the stream, RT safe */
int lm_stream_get_time_n(struct lm_stream *stream, struct pw_time *time, size_t size);

/** Get the current time in nanoseconds. This value can be compared with
 * the \ref pw_time.now value. RT safe. Since 1.1.0 */
uint64_t lm_stream_get_nsec(struct lm_stream *stream);

/** Get the data loop that is doing the processing of this stream. This loop
 * is assigned after lm_stream_connect().  * Since 1.1.0 */
struct pw_loop *lm_stream_get_data_loop(struct lm_stream *stream);

/** Query the time on the stream, deprecated since 0.3.50,
 * use lm_stream_get_time_n() to get the fields added since 0.3.50. RT safe. */
SPA_DEPRECATED
int lm_stream_get_time(struct lm_stream *stream, struct pw_time *time);

/** Get a buffer that can be filled for playback streams or consumed
 * for capture streams. RT safe. */
struct pw_buffer *lm_stream_dequeue_buffer(struct lm_stream *stream);

/** Submit a buffer for playback or recycle a buffer for capture. RT safe. */
int lm_stream_queue_buffer(struct lm_stream *stream, struct pw_buffer *buffer);

/** Return a buffer to the queue without using it. This makes the buffer
 * immediately available to dequeue again. RT safe. */
int lm_stream_return_buffer(struct lm_stream *stream, struct pw_buffer *buffer);

/** Activate or deactivate the stream */
int lm_stream_set_active(struct lm_stream *stream, bool active);
 ...
 * \ref lm_stream_set_active(). A flush without a drain is mostly useful afer
 * a state change to PAUSED, to flush any remaining data from the queues and
 * the converters. RT safe. */
int lm_stream_flush(struct lm_stream *stream, bool drain);

/** Check if the stream is driving. The stream needs to have the
 * LM_STREAM_FLAG_DRIVER set. When the stream is driving,
 * lm_stream_trigger_process() needs to be called when data is
 ...
 * driving according to \ref lm_stream_is_driving(), then it should
 ...
 * events with \ref lm_stream_emit_event() or indirectly with
 * \ref lm_stream_trigger_process() to suggest a new graph cycle
 * to the driver.
 *
 ...
 *
 * Streams that are not drivers according to \ref lm_stream_is_driving()
 ...
 * \ref lm_stream_is_lazy(), this might result in a graph cycle by the
 ...
 *
 * Since 0.3.34 */
int lm_stream_trigger_process(struct lm_stream *stream);

/** Emit an event from this stream. RT safe.
 * Since 1.2.6 */
int lm_stream_emit_event(struct lm_stream *stream, const struct spa_event *event);
 ...
 * Since 1.4.0 */
int lm_stream_set_rate(struct lm_stream *stream, double rate);
